...
async def get_race_predictions(user: dict = Depends(auth_user)):
    ...
    return {
        ...
        "methodology": {
            ...
            "prediction_model": "Performance Curve V2 from qualified observed performances only (no synthetic source): log(T)=log(A)+k·log(D), with single-performance fallback to Riegel k=1.06. Exactly two observations use deterministic shrinkage toward k=1.06 based on evidence strength. For >=3 observations, robust Huber reweighting is followed by a final refit on final weights. If k is outside RunIndex guardrails [1.0, 1.25], fallback k=1.06 is applied and A is re-estimated with fixed slope and weighted intercept. Extrapolation uses symmetric ratio max(target/obs, obs/target) with RunIndex policy thresholds 4.5 (very uncertain) and 6.0 (null prediction). No VMA required for predictions.",
