...
async def get_race_predictions(user: dict = Depends(auth_user)):
    ...
    return {
        ...
        "methodology": {
            ...
            "prediction_model": "Performance Curve V2 from qualified observed performances only (no synthetic source): log(T)=log(A)+k·log(D), with single-performance fallback to Riegel k=1.06. The same curve serves 5K/10K/Semi/Marathon; confidence reflects symmetric extrapolation and fit quality. No VMA required for predictions.",
