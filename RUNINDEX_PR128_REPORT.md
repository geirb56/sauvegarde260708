...
- `terra_integration._workout_to_v2_activity()` ne fallback plus jamais un type absent/inconnu vers `running`.
- `terra_integration.computeRecoveryScore()` n’injecte plus de `training_load_score=70` fictif quand la charge manque.
...
- Aucun workout Terra sans type ne devient artificiellement une course.
- Aucune charge Terra absente ne devient un score legacy neutre fictif (`training_load_score=70`).
