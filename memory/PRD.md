...
## 2026-06 — Pull copilot/dev — PR #182 (migration stats dashboard → DomainActivity, retrait consumers legacy)
- Fetch + merge `sauvegarde/copilot/dev` (f732269→cb7ade0). Merge ORT propre, 0 conflit. `.env` intacts, PRD/rapports préservés.
- PR #182 = stats visibles dashboard migrées vers DomainActivity, suppression logique legacy load signal (server.py +114, Dashboard.jsx +74). Ajoute un `jest.mock("@/components/ui/dialog")` dans les 3 tests dashboard → RÉSOUT le blocage env radix `.tsx` historique.
- Validation (agent-tested) : imports OK, backend + frontend + 4 workers RUNNING, boot propre. Smoke **5/5 = 200** (run-index, dashboard, today, v2/week, v2/cycle). Nouveau `test_dashboard_insight_pr182.py` = **7 passed**. Régression suites PR = **255 passed**. **Jest dashboard = 57 passed (3 suites)** couvrant #178 Readiness V2 (15 scénarios) + #182. Dashboard rend avec 0 erreur JS ; TODAY « Unavailable » désormais en GRIS (fix #178 visible) ; piliers null = `—`.
  - `test_dashboard_insight.py` (ancien) échoue en pytot (13-17) : tests d'intégration sans auth (`Authentication required`) et URL relative si REACT_APP_BACKEND_URL non set → env/legacy, PAS une régression #182.
