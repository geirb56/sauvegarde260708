...
// Converts a snake_case workout type to a camelCase i18n key suffix.
// e.g. "long_easy" → "LongEasy", used as "workoutLongEasy"
function workoutTypeToLabelKey(workoutType) {
  const first = workoutType.charAt(0).toUpperCase();
  const rest = workoutType.slice(1);
  const camel = rest.replace(/_([a-z])/g, function (_, c) { return c.toUpperCase(); });
  return "workout" + first + camel;
}

// Capitalises the first letter of an intensity class for its i18n key.
// e.g. "high" → "High", used as "intensityHigh"
function intensityClassToLabelKey(intensityClass) {
  return "intensity" + intensityClass.charAt(0).toUpperCase() + intensityClass.slice(1);
}
...
function SessionCard({ session, t, unitSystem }) {
  ...
  const workoutLabelKey = workoutTypeToLabelKey(session.workout_type);
  const workoutI18nKey = "trainingV2." + workoutLabelKey;
  const workoutLabel = t(workoutI18nKey) !== workoutI18nKey ? t(workoutI18nKey) : session.workout_type;

  const intensityLabelKey = intensityClassToLabelKey(session.intensity_class);
  const intensityI18nKey = "trainingV2." + intensityLabelKey;
  const intensityLabel = t(intensityI18nKey) !== intensityI18nKey ? t(intensityI18nKey) : session.intensity_class;
